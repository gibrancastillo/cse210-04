...
class Hider:
    ...
    def get_hint(self):
        ...
        if self._distance[-1] == 0:
            ...
            # the current distance [-1] is greater than the previous distancen [-2] the seeker is moving farther away
            hint = "(^.^) Getting colder!"
        elif self._distance[-1] < self._distance[-2]:
            # the current distance [-1] is less than the previous distance [-2] the seeker is getting closer
            hint = "(>.<) Getting warmer!"
        return hint
